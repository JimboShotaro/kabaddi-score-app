...
- リポジトリのルートは `kabaddi_app/`（このフォルダがGit管理対象）
- このファイル（`../設計図.md`）はリポジトリ外にあるため、GitHub保存用に **同内容を `kabaddi_app/docs/設計図.md` にも反映**して管理する
...
- [ ] 未対応（これから実装する）
- [x] 対応済み（実装・動作確認まで完了）
	- ※この「x」は環境によって「×」に見えます（チェック済みの意味）
	- 迷い防止のため、今後はチェックボックス記法（[ ] / [x]）で表記を統一します

### Phase 1〜3（アプリ側の完成度アップ）
- [x] スコア/レイドの詳細ログ（RaidLog）の永続化（履歴から試合の詳細を再閲覧できる形）
- [x] 途中終了/試合終了の扱い整理（完了フラグ、終了時刻、結果の確定）
- [ ] UI操作の一貫性確認（レイダー未選択時の導線、選択リセット、誤操作防止）
	- 実装済み: レイダー未選択時の守備選択ブロック / レイダー変更で守備選択・ボーナスをリセット / 「空レイド」無効化 など
	- TODO: 実機/エミュレータで通し確認
- [ ] パフォーマンス確認（起動直後にフレームスキップ警告が出るため、重い処理の有無を点検）

#### 追加（運用向け改善）
- [x] 新規試合作成で選手名を編集できる
- [x] 控え（ベンチ）選手の作成と表示、交代（長押し→控え選択）
- [x] 保存済み名簿から選手名を選択（オートコンプリート）
- [ ] 名簿管理（削除/整理/チーム別名簿など） ※必要なら

### Phase 4（バックエンド / 同期）
- [x] FastAPI サーバー雛形（/api/v1/sync, /api/v1/analysis/{team_id}）
- [x] 同期用DTO（Pydantic）と Flutter 側のJSON送信（手動同期）
	- バックエンド単体スモークテストOK（/health, /api/v1/sync, /api/v1/analysis, JSONL追記）
	- TODO: Flutterアプリの「手動同期」ボタン → サーバー到達のE2E確認
- [ ] ローカル保存方式の方針決定
	- SharedPreferences継続（軽量・手軽）
	- or Isar再導入（大量データ/検索/耐久性）

### Phase 5（Google Play公開）
- [ ] 公開準備TODOの消化（ルートの `googlePlay公開用手続き.md` を参照）
- [ ] Android公開要件の棚卸し（applicationId / version / targetSdk / 権限 / 依存SDK）
	- [x] applicationId を `jp.shotaroJimbo.kabaddi` に変更

- [ ] iOS/macOS公開要件の棚卸し（bundle id / version / 署名）
	- [x] bundle id を `jp.shotaroJimbo.kabaddi` に変更（iOS/macOS）
- [ ] リリース署名（Play App Signing/アップロードキー）設定
	- [x] 署名設定の仕組み（`android/key.properties` 読み込み）を実装
	- [ ] アップロードキー（keystore）作成
	- [ ] `android/key.properties` 作成（コミットしない）
	- [ ] 署名付き `flutter build appbundle --release` を確認
	- [ ] Play Console 側で Play App Signing を有効化（アップロードキー登録）
- [ ] ストア素材・掲載文・プライバシーポリシーURLの準備
- [ ] Internal/Closedテスト運用 → Production access → 段階的ロールアウト

#### Phase 4 計画（手動同期 / Ubuntu / Oracle Cloud想定）

- 目的: アプリの「手動同期」ボタンで、終了した試合データを FastAPI に送れる状態にする
- バックエンド配置: KABADDI 直下に `kabaddi_app_backend_server/` を作成し、ここで FastAPI を管理する
- 開発フロー（最小）
	- 1) ローカル（Windows）で FastAPI を起動して疎通（Swagger UI / curl / Flutter）
	- 2) Ubuntu（Oracle Cloud）で venv + uvicorn 起動を確認
	- 3) 公開方法を確定（まずは uvicorn 直、次に systemd、必要なら Nginx でリバプロ）
- API（暫定）
	- `POST /api/v1/sync` : 試合1件を受け取り保存
	- `GET /api/v1/analysis/{team_id}` : 指定チームの勝敗簡易集計
- セキュリティ（後続）
	- まずは手動同期のみ（認証なし）で動作確認
	- 次に ID/パスワード or APIキー（ヘッダ）を追加して保護
