...
void main() {
  ...
  group('MatchEngine - 復活ロジック', () {
    ...
    test('復活は先にアウトになった選手から順番に', () {
      ...
      // 先にアウトになった選手が復活
      final player0 = newState.teamA.players.firstWhere(
        (p) => p.id == teamA.players[0].id,
      );
      final player1 = newState.teamA.players.firstWhere(
        (p) => p.id == teamA.players[1].id,
      );
