...
- 依存SDK（`pubspec.yaml`）
    - 解析/広告/クラッシュ収集: **現状なし**（Firebase/AdMob 等は未導入）
    - 通信: `http`（手動同期のみ）
    - ローカル保存: `shared_preferences`
- 権限
    - AndroidManifest: 明示的な permissions なし（位置情報/連絡先/カメラ等なし）
    - iOS Info.plist: 位置情報/カメラ等の利用目的キーなし

##### 重要: 「手動同期」機能をリリースに含めるか

ホームに「手動同期（サーバーへ送信）」があり、ユーザー操作で FastAPI へ送信します。

- 同期の送信内容（`SyncRepository`）
    - 試合ID、日時
    - チーム名（team_a_name / team_b_name）
    - 最終スコア（final_score_a / final_score_b）
    - レイドログ（raid_number / raider_id / outcome / points_gained）

上記を踏まえて、Play Console の回答は次のどちらかで揃えるのが安全です。

- [x] A案（採用: リリースでは同期を出さない/無効化する）
    - 「収集するデータ」: なし
    - 「共有するデータ」: なし
    - 実装対応: リリースビルドでは手動同期ボタンを非表示（debug/profileのみ表示）

- B案（リリースでも手動同期を出す）
    - 「収集するデータ」: あり（ユーザーが操作した場合のみ）
        - カテゴリ候補: App activity（試合結果/レイド結果）、Other（チーム名）
        - 目的: App functionality（同期/分析のため）
        - 収集: 任意（ユーザーが「送信する」を押した場合のみ）
        - 共有: なし（第三者SDKへの送信なし）
    - 「転送中の暗号化」
        - 注意: 現状 `http://...` なので **暗号化なし**。リリースで公開サーバーへ送るなら HTTPS 化を推奨
...
- [x] 広告の有無を申告
    ...
    - 注意: 現状 AdMob 等の広告SDKを入れていない場合、実装と申告が不一致になるため「いいえ」を選択
